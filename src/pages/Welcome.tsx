...
export default function Welcome() {
  ...
  const handleRunSupplyPipeline = async () => {
    ...
    try {
      ...
      const currentIncludes = data.includes || [];

      const reorderedList: string[] = [];
      const priceIncreasedList: string[] = [];

      currentMaterials.forEach((mat: any) => {
        const matId = mat.r_id !== undefined ? mat.r_id : (mat.R_id !== undefined ? mat.R_id : null);
        const matName = mat.r_name || mat.R_name || "Unknown Material";
        const currentStock = Number(mat.stock_quantity !== undefined ? mat.stock_quantity : (mat.Stock_Quantity || 0));

        if (matId === null) return;

        // קטגוריה 1: חומרים שהוזמנו מחדש
        let lookLikeReordered = false;
        if (currentStock >= 100 && currentStock < 150) {
          reorderedList.push(matName);
          lookLikeReordered = true;
        }

        // ספירת כמות ההזמנות הכללית בטבלת INCLUDES
        const totalOrdersCount = currentIncludes.filter((inc: any) => {
          const incMatId = inc.r_id !== undefined ? inc.r_id : inc.R_id;
          return Number(incMatId) === Number(matId);
        }).length;

        // קטגוריה 2: חומרים שמחירם עלה (רק אם עבר חידוש מלאי וגם פופולרי מעל 5 הזמנות)
        if (lookLikeReordered && totalOrdersCount > 5) {
          priceIncreasedList.push(matName);
        }
      });
